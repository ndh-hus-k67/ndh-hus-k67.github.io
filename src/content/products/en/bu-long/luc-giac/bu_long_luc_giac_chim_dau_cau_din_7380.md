---
title: "Button Head Socket Screw (DIN 7380)"
slug: "button-head-socket-din-7380"
description: "Button head socket screws to DIN 7380 with a rounded head for aesthetic, low-profile applications."
updatedAt: 2026-01-19
category:
  slug: "bu-long"
  name: "Bolts"
productType:
  slug: "hex-socket"
  name: "Hex Socket Screws"
filters:
  material: ["inox-201", "inox-304", "inox-316", "inox-316l", "thep-carbon"]
  diameter: ["M3", "M4", "M5", "M6", "M8", "M10", "M12", "M16", "M20"]
  length_mm: []
  surface: ["khong-xu-ly"]
  application: ["tham-my", "noi-that", "co-khi", "lap-rap"]
tags:
  [
    "bu-long-dau-cau",
    "din-7380",
    "button-head",
    "bu-long-luc-giac-chim",
    "bu-long-inox-7380",
    "dau-cau-tham-my",
  ]
seo:
  title: "Bu lông lục giác chìm đầu cầu DIN 7380 - Button head socket Inox"
  description: "Bu lông lục giác chìm đầu cầu DIN 7380 inox 201/304/316/316L. Đầu cầu bo tròn thẩm mỹ, siết bằng lục lăng. Kích thước M3-M20, phù hợp nội thất, cơ khí."
  keywords:
    [
      "bu lông din 7380",
      "button head socket",
      "bu lông đầu cầu",
      "bu lông lục giác chìm đầu cầu",
      "bu lông inox 7380",
    ]
variants:
  - sku: "BL7380-M6-20-INOX304"
    name: "Bu lông lục giác chìm đầu cầu M6×20 Inox 304"
    material: "inox-304"
    diameter: "M6"
    length_mm: 20
  - sku: "BL7380-M8-25-INOX304"
    name: "Bu lông lục giác chìm đầu cầu M8×25 Inox 304"
    material: "inox-304"
    diameter: "M8"
    length_mm: 25
  - sku: "BL7380-M10-30-INOX316"
    name: "Bu lông lục giác chìm đầu cầu M10×30 Inox 316"
    material: "inox-316"
    diameter: "M10"
    length_mm: 30
  - sku: "BL7380-M12-40-INOX316L"
    name: "Bu lông lục giác chìm đầu cầu M12×40 Inox 316L"
    material: "inox-316l"
    diameter: "M12"
    length_mm: 40
breadcrumbs:
  - name: "Home"
    url: "/"
  - name: "Bolts"
    url: "/bu-long"
  - name: "Hex Socket"
    url: "/bu-long/bu-long-luc-giac"
  - name: "Button Head (DIN 7380)"
    url: "/bu-long/bu-long-luc-giac/bu-long-inox-luc-giac-chim-dau-cau-din-7380"
relatedCategory:
  - slug: "dai-oc"
    name: "Đai ốc"
  - slug: "vit"
    name: "Vít"
  - slug: "inox-304"
    name: "Inox 304"
main:
  id: 14
  content: |
    Button head socket screws (DIN 7380) have a smooth rounded head with an internal hex for Allen key installation. They provide a neat, low-profile appearance suitable for visible assemblies.

    Available in stainless grades or carbon steel depending on corrosion resistance and strength needs.
  imgCard: "@/images/products/bu-long/luc-giac/bu-long-luc-giac-chim-dau-cau.avif"
  imgMain: "@/images/products/bu-long/luc-giac/bu-long-luc-giac-chim-dau-cau.avif"
  imgAlt: "Bu lông inox lục giác chìm đầu cầu DIN 7380"

tabs:
  - id: "tabs-with-card-item-1"
    dataTab: "#tabs-with-card-1"
    title: "Mô tả"
  - id: "tabs-with-card-item-2"
    dataTab: "#tabs-with-card-2"
    title: "Thông số kỹ thuật"
  - id: "tabs-with-card-item-3"
    dataTab: "#tabs-with-card-3"
    title: "Bản vẽ"

longDescription:
  title: "DIN 7380 – Button Head for Clean, Rounded Finish"
  subTitle: |
    DIN 7380 screws produce a smooth rounded finish and are preferred where aesthetics and reduced snagging are important. Choose stainless materials (304/316) for corrosive environments.
  btnTitle: "Contact sales for consultation"
  btnURL: "/contact"

descriptionList:
  - title: "Đầu Cầu Thẩm Mỹ"
    subTitle: "Bề mặt bo tròn giúp tổng thể mối ghép gọn, phù hợp các vị trí cần độ hoàn thiện cao."
  - title: "Siết Bằng Lục Lăng"
    subTitle: "Lỗ lục giác chìm giúp thao tác thuận tiện trong không gian hẹp."
  - title: "Vật Liệu Linh Hoạt"
    subTitle: "Tùy chọn inox 201/304/316/316L hoặc thép để cân bằng chống ăn mòn và chi phí."

specificationsLeft:
  - title: "Tiêu chuẩn"
    subTitle: "DIN 7380"
  - title: "Kiểu đầu"
    subTitle: "Đầu cầu (Button Head), lục giác chìm"
  - title: "Vật liệu"
    subTitle: "Inox 201/304/316/316L; Thép"
  - title: "Cấp bền (tham khảo)"
    subTitle: "4.6 – 8.8 (tùy vật liệu/lô hàng), hoặc theo cấp inox tương ứng"
  - title: "Kích thước"
    subTitle: "Tham khảo: M6 ~ M30 (có thể gặp các cỡ khác tùy tiêu chuẩn/nhu cầu)"

tableData:
  - feature: ["Thông số", "Giá trị"]
    description:
      - ["Tên gọi", "Bu lông inox lục giác chìm đầu cầu DIN 7380"]
      - ["Tiêu chuẩn", "DIN 7380"]
      - ["Kiểu đầu", "Đầu cầu, lục giác chìm"]
      - ["Vật liệu", "Inox 201/304/316/316L; Thép"]
      - ["Giá", "Liên hệ để nhận báo giá tốt nhất"]

  - feature:
      [
        "d",
        "M3",
        "M4",
        "M5",
        "M6",
        "M8",
        "M10",
        "M12",
        "M14",
        "M16",
        "M18",
        "M20",
        "M22",
        "M24",
        "M27",
        "M30",
      ]
    description:
      - [
          "P (bước ren)",
          "0,5",
          "0,7",
          "0,8",
          "1",
          "1,25",
          "1,5",
          "1,75",
          "–",
          "2",
          "–",
          "–",
          "–",
          "–",
          "–",
          "–",
        ]
      - [
          "dk",
          "5,7",
          "7,6",
          "9,5",
          "10,5",
          "14",
          "17,5",
          "21",
          "–",
          "28",
          "–",
          "–",
          "–",
          "–",
          "–",
          "–",
        ]
      - [
          "k",
          "1,65",
          "2,2",
          "2,75",
          "3,3",
          "4,4",
          "5,5",
          "6,6",
          "–",
          "8,8",
          "–",
          "–",
          "–",
          "–",
          "–",
          "–",
        ]
      - [
          "s",
          "2",
          "2,5",
          "3",
          "4",
          "5",
          "6",
          "8",
          "–",
          "10",
          "–",
          "–",
          "–",
          "–",
          "–",
          "–",
        ]

blueprints:
  first: "@/images/blueprint-1.avif"
  second: "@/images/blueprint-2.avif"
---
